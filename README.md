...
- **DevOps (Ansible, Terraform, YAML)**: `ansiblels`, `terraformls`, `yamlls`, `terraform_fmt` & `yamlfmt`.
...
> **Note:** `nvim-autopairs` (bracket matching), `nvim-lint` (independent linter), and `nvim-dap` (debugging) are currently _temporarily disabled_ in the config.
...
> **Lưu ý:** Các chức năng `nvim-autopairs` (tự đóng ngoặc), `nvim-lint` (linter độc lập), và `nvim-dap` (gỡ lỗi) hiện đang được _tạm thời vô hiệu hóa_ trong cấu hình hiện tại.
